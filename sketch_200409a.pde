...
void setup() {
  ...
  //    chunk[i][(int)altura][j] = block.DIRT.getBlock();
...
void cubo(float x, float y, float z, color c) {
  ...
  // ABAJO
  if (y + 1 == chunkHeight || (y >= 0 && y < chunkHeight && chunk[(int)x][(int)y + 1][(int)z] == block.AIR.getBlock())) {
    caraAbajo(x, y, z);
  }
  ...
  // DERECHA
  if (z + 1 == chunkSize || (z >= 0 && z < chunkSize && chunk[(int)x][(int)y][(int)z + 1] == block.AIR.getBlock())) {
    caraDerecha(x, y, z); 
  }
  ...
  // FRENTE
  if (x + 1 == chunkSize || (x >= 0 && x < chunkSize && chunk[(int)x + 1][(int)y][(int)z] == block.AIR.getBlock())) {
    caraFrente(x, y, z); 
  }
